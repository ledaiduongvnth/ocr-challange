...
class BaseLLMComplexBlockProcessor(BaseLLMProcessor):
    ...
    def rewrite_blocks(self, document: Document):
        ...
        # print("using rewrite BaseLLMComplexBlockProcessor ")
        total_blocks = sum(
            len(page.contained_blocks(document, self.block_types))
            for page in document.pages
        )
        print("total block", total_blocks)
        if total_blocks == 0:
            return

        pbar = tqdm(
            total=total_blocks,
            desc=f"__init__.py {self.__class__.__name__} running",
...
class BaseLLMSimpleBlockProcessor(BaseLLMProcessor):
    ...
    """
    print ("using BaseLLMSimpleBlockProcessor")
