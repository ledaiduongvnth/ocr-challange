...
# print("LLMTableProcessor module imported")
...
class LLMTableProcessor(BaseLLMComplexBlockProcessor):
    print("*****************************************************using llm to process table **********************************************************************************")
...
"""
    ...
    def process_rewriting(self, document: Document, page: PageGroup, block: Table):
        """
        làm nhiệm vụ chia nhỏ bảng, 
        gửi từng phần vào mô hình LLM để “sửa”, 
        sau đó ghép kết quả lại để tạo bảng cuối cùng đã được hiệu chỉnh
        """
        children: List[TableCell] = block.contained_blocks(
            document, (BlockTypes.TableCell,)
        )
        with open("/media/khaint02/DATA1/data_khai/anh_duong/marker/output/output2/children.txt", "w", encoding="utf-8") as f:
                for c in children:
                    f.write(repr(c) + "\n")
        print("save children")
        ...
        if not children:
            ...
            print(" Happens if table/form processors didn't run")
            return
...
#KHAI 
            ...
            html_file_path = f"/media/khaint02/DATA1/data_khai/anh_duong/marker/output/output2/table_batch_{i}.html"
            with open(html_file_path, "w", encoding="utf-8") as f:
                f.write(block_html)
            print(f"save html before using llm in path {html_file_path}")
#
            batch_cell_bboxes = [
...
#khai 
            x_min = min([bbox[0] for bbox in batch_cell_bboxes])
            y_min = min([bbox[1] for bbox in batch_cell_bboxes])
            x_max = max([bbox[2] for bbox in batch_cell_bboxes])
            y_max = max([bbox[3] for bbox in batch_cell_bboxes])

            pad = 2  # pixels
            x_min = max(x_min - pad, 0)
            y_min = max(y_min - pad, 0)
            x_max = min(x_max + pad, block_image.size[0])
            y_max = min(y_max + pad, block_image.size[1])

            batch_image = block_image.crop((x_min, y_min, x_max, y_max))
#
            # batch_image = block_image.crop(batch_bbox)
            block_html = block.format_cells(document, [], None, batch_cells)
            # print(f"=== HTML sent to LLM (batch {i // self.max_rows_per_batch}) ===\n{block_html}")
            batch_image = self.handle_image_rotation(batch_cells, batch_image)
            batch_image.save(f"/media/khaint02/DATA1/data_khai/anh_duong/marker/output/output2/table_batch_{i}.png")
            print(f"save img in /media/khaint02/DATA1/data_khai/anh_duong/marker/output/output2/table_batch_.png")
            ...
            with open("/media/khaint02/DATA1/data_khai/anh_duong/marker/output/output2/parsed_cells.txt", "w", encoding="utf-8") as f:
                for c in parsed_cells:
                    f.write(repr(c) + "\n")

            with open("/media/khaint02/DATA1/data_khai/anh_duong/marker/output/output2/cell.txt", "w", encoding="utf-8") as f:
                f.write(repr(cell) + "\n")

            print("Saved parsed_cells and cell to TXT files.")
            # print("parsed_cells", parsed_cells)
            # print("cell", cell)
    ...
    def rewrite_single_chunk(
        ...
        """
        Nó gửi một phần bảng (HTML + hình ảnh) đến mô hình LLM để yêu cầu chỉnh sửa bảng,
          nhận lại bảng HTML đã được hiệu chỉnh, và chuyển đổi kết quả đó trở lại thành các TableCell 
          (ô bảng) dùng trong document.
          """
    ...
    def get_cell_text(element, keep_tags=("br", "i", "b", "span", "math")) -> str:
        """lấy text bên trong một ô HTML, loại bỏ các thẻ không cần thiết, 
        giữ nguyên một vài thẻ đặc biệt giúp định dạng hoặc chứa công thức.
        """
    ...
    def parse_html_table(
        ...
        """
        có nhiệm vụ chuyển đổi một bảng HTML (do LLM trả về) 
        → thành danh sách các đối tượng TableCell có tọa độ, hàng, cột, rowspan, colspan
        """
        print("block", block)
